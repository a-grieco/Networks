...
bool get_parsed_data(std::string client_msg, std::string& webserv_host,
  ...
  std::string host, path, port, body;
  std::vector<std::string> headers;
  Parse_Error err;

  // if parse successful, generate web server request and return true
  if(parse_client_msg(client_msg, host, path, port, headers, body, err)) {
    webserv_host = host;
    webserv_port = port;
    if(DEBUG_MODE) {
...
bool parse_client_msg(std::string msg, std::string& host, std::string& path,
  ...
  if(parse_request_line(req, host, path, port, err)) {
    ...
    if(msg.size() > 0) {
      printf("ASSIGNING MESSAGE TO BODY: \n", body.c_str());
      body = msg;
    }
    else { printf("NOTHING LEFT IN MESSAGE FOR BODY\n"); }
